...
void _abortError(const char* msg, const char* filename, const char* fname,
                 int line)
{
...
float** generate_kernel(float** kernel, size_t ker_size)
{
    ...
    for (size_t x = 0; x < ker_size; ++x)
    {
        ...
        {
            kernel[x][y] = std::exp(-0.5
                                    * (std::pow((x - mean) / sigma, 2.0)
...
void pxGaussianBlur(matrixImage<float>* buf_in, matrixImage<float>* buf_out,
                    size_t x, size_t y, float** ker, size_t ker_size,
                    const size_t offset)
{
    float px = 0;
...
float pxDilationErosion(matrixImage<float>* matImg, const size_t w,
                        const size_t h, const size_t se_w, const size_t se_h,
                        const bool d_or_e)
{
    /*
...
void dilationErosion(matrixImage<float>* mat_in, matrixImage<float>* mat_out,
                     const size_t se_w, const size_t se_h, const bool d_or_e)
{
    /*
     * d_or_e== true => dilation, else erosion
     */
...
void pxBernsenThreshold(matrixImage<float>* mat_in, matrixImage<float>* mat_out,
                        size_t se_w, size_t se_h, float contrast_threshold,
                        size_t w, size_t h, size_t off_w, size_t off_h,
                        float avg_intensity)
{
...
void bernsenThreshold(matrixImage<float>* mat_in, matrixImage<float>* mat_out,
                      size_t se_w, size_t se_h, float contrast_threshold = 15.f)
{
...
void otsuThreshold(matrixImage<float>* mat_in, matrixImage<float>* mat_out)
{
    ...
    int mean_intensity =
        find_mean_intensity(histo, mat_in->width * mat_in->height);
...
void update_neighbor(matrixImage<float>* mat_in, matrixImage<float>* mat_out,
                     size_t w, size_t h, std::queue<size_t>& q, float cur_label)
{
...
void update_neighbors(matrixImage<float>* mat_in, matrixImage<float>* mat_out,
                      size_t w, size_t h, std::queue<size_t>& q,
                      float cur_label)
{
    if (w > 0)
            ...
            const char* filename, json& bboxes)
{
    ...
    matrixImage<float>* matGray1 =
        new matrixImage<float>(matImg1->width, matImg1->height);
    ...
    matrixImage<float>* matGray2 =
        new matrixImage<float>(matImg2->width, matImg2->height);
    ...
    matrixImage<float>* matThreshold2 =
        new matrixImage<float>(matImg1->width, matImg1->height);
