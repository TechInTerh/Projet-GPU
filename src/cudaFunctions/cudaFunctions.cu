...
void* cudaMallocX(size_t size)
{
    ...
    if (err != cudaSuccess)
    {
        spdlog::error("Error allocating memory in GPU: {}",
                   ...
                   size_t width, size_t height, cudaMemcpyKind kind)
{
    ...
    if (err != cudaSuccess)
    {
        spdlog::error("Error copying memory in {}: {}",
                      cudaMemcpyKindToStr(kind), cudaGetErrorString(err));
        std::exit(1);
    }
}
...
void cudaFreeX(void* ptr)
{
    ...
    if (err != cudaSuccess)
    {
        spdlog::error("Error freeing memory in GPU: {}",
                      cudaGetErrorString(err));
        std::exit(1);
    }
}
