...
__global__ void grayscale(uchar3* matImg, float* matOut, size_t width,
...
__global__ void gaussianBlur(float* matIn, float* matOut, size_t width,
                             size_t height, size_t pitch_in, size_t pitch_out,
                             size_t kernel_size, float* kernel,
                             size_t kernel_pitch)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    size_t idy = blockIdx.y * blockDim.y + threadIdx.y;
...
{
    matrixImage<float>* kernel =
        new matrixImage<float>(kernel_size, kernel_size);
                             ...
                             dim3 threads, dim3 blocks)
{
    ...
    matrixImage<float>* matGray =
        new matrixImage<float>(matImg->width, matImg->height);
    matGray->toGpu();
    grayscale<<<blocks, threads>>>(matImg->buffer, matGray->buffer,
    ...
    for (int i = 0; i < numberBlur; i++)
    {
        gaussianBlur<<<blocks, threads>>>(
            matGray->buffer, matBlur->buffer, matGray->width, matGray->height,
            matGray->pitch, matBlur->pitch, kernel->width, kernel->buffer,
            kernel->pitch);
        cudaDeviceSynchronizeX();
        if (i != numberBlur - 1)
        {
