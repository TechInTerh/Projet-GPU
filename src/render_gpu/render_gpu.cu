...
__global__ void absDiff(float* matIn, float* matOut, size_t width,
...
/**
 * @brief Compute the absolute difference between two matrix, stocking the
 * result in "@param matBlur2"
 * @param matBlur1
 * @param matBlur2
 * @param threads
 * @param blocks
...
__global__ void dilatationErosion(float* matIn, float* matOut, size_t width,
                                  ...
                                  size_t pitch_out, size_t se_w, size_t se_h,
                                  bool d_or_e)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    size_t idy = blockIdx.y * blockDim.y + threadIdx.y;
...
void launchThreshold(matrixImage<float>* matIn, dim3 threads, dim3 blocks)
{
    ...
    thresholding<<<blocks, threads>>>(matIn->buffer, matIn->width,
                                      matIn->height, matIn->pitch, mean);
    cudaDeviceSynchronizeX();
    cudaFreeX(gpu_histo);
}

__global__ void setOnIndex(float* matIn, float* matOut, size_t width,
                           size_t height, size_t pitch_in, size_t pitch_out,
                           int* index)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    size_t idy = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void labelNeighbors(float* matIn, float* matOut, size_t width,
                               size_t height, size_t pitch_in, size_t pitch_out,
                               int* isChanged)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (size_t sw = 0; sw <= 2; sw++)
    {
        for (size_t sh = 0; sh <= 2; sh++)
        {
            ...
            {
                float* px_in =
                    eltPtrFloat(matIn, idx + sw - 1, idy + sh - 1, pitch_in);
                if (*px_in != 0.f)
                {
                    min_value = my_min(min_value, *px_in);
                }
                               ...
                               dim3 blocks)
{
    ...
    dilatationErosion<<<blocks, threads>>>(
        matIn->buffer, matOut->buffer, matIn->width, matIn->height,
        matIn->pitch, matOut->pitch, size1_w, size1_h, true);
    cudaDeviceSynchronizeX();
    dilatationErosion<<<blocks, threads>>>(
        matOut->buffer, matIn->buffer, matIn->width, matIn->height,
        matIn->pitch, matOut->pitch, size1_w, size1_h, false);
    cudaDeviceSynchronizeX();
    spdlog::info("Launching morph opening");
    dilatationErosion<<<blocks, threads>>>(
        matIn->buffer, matOut->buffer, matIn->width, matIn->height,
        matIn->pitch, matOut->pitch, size2_w, size2_h, false);

    cudaDeviceSynchronizeX();
    dilatationErosion<<<blocks, threads>>>(
        matOut->buffer, matIn->buffer, matIn->width, matIn->height,
        matIn->pitch, matOut->pitch, size2_w, size2_h, true);
    cudaDeviceSynchronizeX();
    delete matOut;
}

matrixImage<float>* launchLabelisation(matrixImage<float>* matIn, dim3 threads,
                                       dim3 blocks)
{
    ...
    setOnIndex<<<blocks, threads>>>(matIn->buffer, matOutIndex->buffer,
                                    matIn->width, matIn->height, matIn->pitch,
                                    matOutIndex->pitch, index);
    cudaDeviceSynchronizeX();
    ...
    while (isChanges >= 1)
    {
        ...
        labelNeighbors<<<blocks, threads>>>(
            matOutIndex->buffer, matOutLabel->buffer, matOutLabel->width,
            matOutLabel->height, matOutIndex->pitch, matOutLabel->pitch,
            isChanged_gpu);
        cudaDeviceSynchronizeX();
...
__global__ void multiplyValue(float* matIn, size_t width, size_t height,
                              size_t pitch_in, float value)
{
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    size_t idy = blockIdx.y * blockDim.y + threadIdx.y;
    if (idx >= width || idy >= height)
            ...
            const char* filename, json& bboxes)
{
    ...
    matrixImage<float>* matLabel =
        launchLabelisation(matBlur2, threads, blocks);
    matLabel->toCpu();
